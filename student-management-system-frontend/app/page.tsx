...
export default function StudentManagementSystem() {
  ...
  const handleStudentScan = async (studentId: string) => {
    ...
    try {
      ...
      // Mark attendance (idempotent via backend duplicate check)
      ...
      // Show toast popup for quick visual feedback
      toast({
        title: "تم تسجيل الحضور",
        description: `${student.name} — الحالة: ${statusText}`,
      });
  ...
  const skipLinks = [
    { href: "#main-content", text: "الانتقال إلى المحتوى الرئيسي" },
    { href: "#sidebar-navigation", text: "الانتقال إلى قائمة التنقل" },
    { href: "#student-grid", text: "الانتقال إلى جدول الطلاب" },
    { href: "#qr-scanner", text: "الانتقال إلى ماسح QR" },
  ];
