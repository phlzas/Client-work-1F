...
impl AttendanceService {
    ...
    pub fn mark_attendance(db: &Database, student_id: &str, date: &str) -> DatabaseResult<AttendanceRecord> {
        // Validate date format
        Self::validate_date_format(date)?;
        
        // Check if student exists
        let student_exists: bool = db.connection().query_row(
        ...
        let existing_count: i32 = db.connection().query_row(
        ...
        if existing_count > 0 {
            return Err(crate::database::DatabaseError::Migration(
                format!("Attendance already recorded for student '{}' on date '{}'", student_id, date)
            ));
        }
        ...
        let attendance_record = stmt.query_row(
            params![student_id, date],
            |row| {
                Ok(AttendanceRecord {
                    id: row.get(0)?,
    ...
    pub fn check_attendance_on_date(db: &Database, student_id: &str, date: &str) -> DatabaseResult<bool> {
        // Validate date format
        Self::validate_date_format(date)?;
    ...
    pub fn get_attendance_history(db: &Database, filter: Option<AttendanceHistoryFilter>) -> DatabaseResult<Vec<AttendanceRecord>> {
        ...
        if let Some(filter) = filter {
            ...
            if let Some(group_name) = filter.group_name {
                conditions.push("student_id IN (SELECT id FROM students WHERE group_name = ?)".to_string());
        ...
        attendance_iter.collect::<Result<Vec<_>, _>>()
            .map_err(|e| crate::database::DatabaseError::Sqlite(e))
    }
    
    /// Get attendance history for a specific student
    pub fn get_student_attendance_history(db: &Database, student_id: &str) -> DatabaseResult<Vec<AttendanceRecord>> {
    ...
    pub fn get_student_attendance_stats(db: &Database, student_id: &str, start_date: Option<&str>, end_date: Option<&str>) -> DatabaseResult<AttendanceStats> {
        ...
        let present_days: i32 = db.connection().query_row(
            &present_query,
            &params_refs[..],
            |row| row.get(0),
        ).map_err(|e| crate::database::DatabaseError::Sqlite(e))?;
        ...
        let enrollment_date: String = db.connection().query_row(
        ...
        let start_calc_date = if let Some(start) = start_date {
            if start > enrollment_date.as_str() { start } else { enrollment_date.as_str() }
        } else {
    ...
    pub fn get_daily_attendance_summary(db: &Database, date: &str, group_name: Option<&str>) -> DatabaseResult<DailyAttendanceSummary> {
        Self::validate_date_format(date)?;
        ...
        let (present_students, present_ids_str): (i32, Option<String>) = db.connection().query_row(
            &present_query,
            &params_refs[..],
    ...
    pub fn get_attendance_summary_range(db: &Database, start_date: &str, end_date: &str, group_name: Option<&str>) -> DatabaseResult<Vec<DailyAttendanceSummary>> {
        Self::validate_date_format(start_date)?;
        Self::validate_date_format(end_date)?;
    ...
    pub fn validate_date_format(date: &str) -> DatabaseResult<()> {
        match NaiveDate::parse_from_str(date, "%Y-%m-%d") {
            Ok(_) => Ok(()),
            Err(_) => Err(crate::database::DatabaseError::Migration(
                format!("Invalid date format '{}'. Expected format: YYYY-MM-DD", date)
            )),
        }
    }
    ...
    fn calculate_days_between(start_date: &str, end_date: &str) -> DatabaseResult<i32> {
        let start = NaiveDate::parse_from_str(start_date, "%Y-%m-%d")
            .map_err(|_| crate::database::DatabaseError::Migration(
                format!("Invalid start date format: {}", start_date)
            ))?;
        
        let end = NaiveDate::parse_from_str(end_date, "%Y-%m-%d")
            .map_err(|_| crate::database::DatabaseError::Migration(
                format!("Invalid end date format: {}", end_date)
            ))?;
...
mod tests {
    ...
    fn test_mark_attendance_duplicate_prevention() {
        ...
        assert!(result2.unwrap_err().to_string().contains("already recorded"));
    }
    ...
    fn test_get_attendance_history() {
        ...
        let student_attendance = AttendanceService::get_attendance_history(&db, Some(filter)).unwrap();
        assert_eq!(student_attendance.len(), 2);
        assert!(student_attendance.iter().all(|a| a.student_id == "student1"));
    }
    ...
    fn test_date_formatting() {
        ...
        assert_eq!(AttendanceService::format_date("2024-01-15").unwrap(), "2024-01-15");
        assert_eq!(AttendanceService::format_date("15/01/2024").unwrap(), "2024-01-15");
        assert_eq!(AttendanceService::format_date("01/15/2024").unwrap(), "2024-01-15");
        assert_eq!(AttendanceService::format_date("15-01-2024").unwrap(), "2024-01-15");
        assert_eq!(AttendanceService::format_date("2024/01/15").unwrap(), "2024-01-15");
    ...
    fn test_daily_attendance_summary() {
        ...
        let summary = AttendanceService::get_daily_attendance_summary(&db, "2024-01-15", None).unwrap();
        ...
        let group_summary = AttendanceService::get_daily_attendance_summary(&db, "2024-01-15", Some("Group A")).unwrap();
    ...
    fn test_delete_attendance() {
        ...
        assert!(AttendanceService::check_attendance_on_date(&db, "student1", "2024-01-15").unwrap());
        ...
        let not_deleted = AttendanceService::delete_attendance(&db, "student1", "2024-01-15").unwrap();
        assert!(!not_deleted);
    }
}